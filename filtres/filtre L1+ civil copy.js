...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateContent(data) {
        ...
        if (data.ouvrages.L1) {
            data.ouvrages.L1.filter(item => item.matiere === 'droit civil').forEach(item => {
        ...
        if (data.cours.L1) {
            data.cours.L1.filter(item => item.matiere === 'droit civil').forEach(item => {
                const div = document.createElement('div');
                div.innerHTML = `
                    <a href="${item.url}">
                        <button>
        ...
        if (data.schemas.L1) {
            data.schemas.L1.filter(item => item.matiere === 'droit civil').forEach(item => {
                const div = document.createElement('div');
